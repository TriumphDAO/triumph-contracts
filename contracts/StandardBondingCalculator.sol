...
contract TriumphBondingCalculator is IBondingCalculator {
    using FixedPoint for *;
    using SafeMath for uint256;

    IERC20 internal immutable TOTC;

    constructor(address _TOTC) {
        require(_TOTC != address(0), "Zero address: TOTC");
        TOTC = IERC20(_TOTC);
    }
    ...
    function markdown(address _pair) external view override returns (uint256) {
        ...
        uint256 reserve;
        if (IUniswapV2Pair(_pair).token0() == address(TOTC)) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair(_pair).token1() == address(TOTC), "Invalid pair");
            reserve = reserve0;
        }
        return reserve.mul(2 * (10**IERC20Metadata(address(TOTC)).decimals())).div(getTotalValue(_pair));
    }
}
