 ...
 */
contract YieldStreamer is IYieldStreamer, YieldSplitter, TriumphAccessControlled {
    ...
    */
    ...
    ) YieldSplitter(sOHM_) TriumphAccessControlled(ITriumphAuthority(authority_)) {
        gOHM = gOHM_;
        OHM = OHM_;
    ...
    */
    function withdrawPrincipal(uint256 id_, uint256 amount_) external override {
        ...
        if (amount_ >= IbTOC(gOHM).balanceTo(depositInfo[id_].principalAmount)) {
    ...
    */
    ...
    function getPrincipalInGOHM(uint256 id_) external view returns (uint256) {
        return IbTOC(gOHM).balanceTo(depositInfo[id_].principalAmount);
    }
